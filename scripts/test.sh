...
if [ $USE_REDIS_DOCKER -eq 1 ]; then
  REDIS_CONTAINER_ID=$(sudo docker run --rm -d -p 6379:6379 redis:8.0.2-alpine --requirepass testpw)
  for i in {1..10}; do
...
if [ $USE_MYSQL_DOCKER -eq 1 ]; then
  MYSQL_CONTAINER_ID=$(sudo docker run --rm -d -p 3306:3306 -e MYSQL_ROOT_PASSWORD=testpw mysql:8.0.42)
  for i in {1..10}; do
...
if [ $USE_POSTGRES_DOCKER -eq 1 ]; then
  POSTGRES_CONTAINER_ID=$(sudo docker run --rm -d -p 5432:5432 -e POSTGRES_PASSWORD=testpw postgres:17.5-alpine)
  for i in {1..10}; do
